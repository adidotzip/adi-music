...
<script lang="ts">
	...
	const processedItems = $derived.by(() => {
		...
		for (let i = 0; i < lines.length; i += 1) {
			...
			if (words.length > 0) {
				// Condition 1: If the backend overrode it to line-only, all words will share the exact same start time
				const allSameTime = words.every(w => w.time === words[0].time)

				if (allSameTime) {
					isLineLevel = true
				} else {
					// Condition 2: Check for purely uniform fake timings generated by basic LRC parsers
					...
					if (faked) {
						for (let w = 1; w < words.length; w++) {
							...
							if (Math.abs(diff - expectedDuration) > 5) {
								faked = false
								break
							}
						}
								...
								<div class="primary-lyrics-block">
									{#each primaryWords as word}
										{#if word.string.trim().length > 0}
											{@const isPastWord = isLinePast || (isActiveLine && (item.isLineLevel || word.originalIndex < activeWordIdx))}
											...
											{@const wordProgress = isLinePast ? 100 : (item.isLineLevel 
												? (isActiveLine ? 100 : 0) 
												: (isCurrentWord 
